...
function drawDodgerSelection() {
    ...
    drawDodgerCard(mouseOver.quasar, highscore.euphoria === 100, quasar, "QUASAR", "EVENT HORIZON", "EUPHORIA 100%", "rgb(230, 152, 9)", "rgb(219, 144, 7)", "rgb(255, 165, 0)");
    ...
    drawAbilityDesc(mouseOver.j_sab, highscore.andromeda === 100, "rgba(255, 0, 0, 0.6)", "rgba(210, 0, 0, 0.9)", "rgba(200, 0, 0, 0.7)", "DASH",
                    ...
                    "Dash Cooldown: 2s");
    drawAbilityDesc(mouseOver.quasar, highscore.euphoria === 100, "rgba(255, 165, 0, 0.7)", "rgba(230, 152, 9, 0.9)", "rgba(201, 135, 12, 0.7)", "EVENT HORIZON",
...
function drawText() { // draws the current time, highest time, and enemy count
    ...
    // Absolute Zero
        ...
        // Absolute Zero CD
        let absoluteZeroCDLeft = ((1000 - (now - absoluteZero.lastEnded)) / 1000).toFixed(2);
    ...
    // Event Horizon
        ...
        if (now - eventHorizon.lastEnded >= 8000) {
            ...
            ctx.fillText(`Active: Event Horizon (${controls[0]})`);
        } else {
            eventHorizon.usable = false;
            ctx.fillText(`Active: Event Horizon (${eventHorizonCDLeft}s)`);
        }
    }
}
